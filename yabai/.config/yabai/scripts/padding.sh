...
TOTAL="$(yabai -m query --windows --space | jq '.[] | select(."is-floating" == false) | select(."is-sticky" == false) | select(."is-visible" == true) | select(."is-minimized" == false) | length % 23 + 1' | wc -l)"
...
if [[ $TOTAL -gt 1 ]]; then
  if [[ $CURRENT -eq 0 ]]; then
    yabai -m space --gap abs:15; yabai -m space --padding abs:14:9:9:9
  fi
else 
